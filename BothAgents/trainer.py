...
def averagePlot(time, step, reward, qmax):
    ...
    if defense == True:
        ...
        agentType = "Offensive"
    print ("Agent:", agentType, "/ Total Frames:", time, "/ Current Reward:", reward, "/ QMax: %e" % qmax)
...
def go(agentType, pswitch, qswitch, bswitch):
    global defense
    if agentType == "1":
        defense = True
        pong.defense = True
    if agentType == "2":
        defense = False
        pong.defense = False
    pong.pswitch = pswitch
    pong.qswitch = qswitch
    pong.bswitch = bswitch
